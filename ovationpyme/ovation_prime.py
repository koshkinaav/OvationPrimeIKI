...
class LatLocaltimeInterpolator(object):
    """
    Класс определяет какого полюса данные
    """
    ...
    def __init__(self, mlat_grid, mlt_grid, var):
        ...
        # Эта строка подсчитывает количество значений в mlat_orig, которые больше нуля (северное полушарие) и меньше
        # нуля (южное полушарие)
    ...
    def interpolate(self, new_mlat_grid, new_mlt_grid, method='nearest'):
        ...
        Функция находит недоопределенные значения на сетки одним из методов интерполяции
        """
        # Эти строки преобразуют широту и localtime в прямоугольные координаты с помощью функции latlt2cart
...
class BinCorrector(object):
    ...
    def fix(self, y_grid, min_mlat=49, max_mlat=75, label=''):
        ...
        if self.dy_thresh is None:
            self.dy_thresh = 3. * np.nanstd(np.diff(y_grid.flatten()))  # дисперсия, умноженная на три
        # Функция wraparound полезна для того, чтобы избежать проблем с граничными условиями или выходом за границы
        # массива при выполнении операций, зависящих от близлежащих значений. Она создает иллюзию того, что массив
        # имеет циклическую или кольцевую структуру.
        ...
        for i_mlat, mlat in enumerate(self.mlats):
            ...
            # Здесь мы находим некорректные бины и в исходном массиве заменяем их интерполированными
...
class ConductanceEstimator(object):
    ...
    http://www.kosmofizika.ru/spravka/toki.htm
    """
    ...
    def get_conductance(self, dt, hemi='N', solar=True, auroral=True, background_p=None, background_h=None,
        ...
        total_sigp_sqrd = np.zeros_like(sigp_solar)  # проводимость Pederson
        total_sigh_sqrd = np.zeros_like(sigh_solar)  # проводимость Hall
    ...
    def prob_estimate(self, dF, i_mlt_bin, i_mlat_bin):
        ...
        if b1 == 0. and b2 == 0.:
            ...
            if p == 0.:
                ...
                i_dFbin_1 = i_dFbin - 1 if i_dFbin > 0 else i_dFbin + 2  # one dF bin before by preference, two after
                # in extremis
                i_dFbin_2 = i_dFbin + 1 if i_dFbin < self.n_dF_bins - 1 else i_dFbin - 2  # one dF bin after by
                # preference, two before in extremis
